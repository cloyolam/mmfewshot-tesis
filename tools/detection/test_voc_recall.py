...
def main():
    ...
    print(f"Base classes: {dataset.get_classes('BASE_CLASSES_SPLIT4')}")
    print(f"Novel classes: {dataset.get_classes('NOVEL_CLASSES_SPLIT4')}")
    ...
    print(f"type(data_loader) = {type(data_loader)}")
    ...
    if cfg.data.get('model_init', None) is not None:
        ...
        if cfg.data.model_init.get('ann_cfg', None) is None:
            print("cfg.data.model_init.get('ann_cfg', None) is None")
        ...
        print("Building model_init_dataset...")
        model_init_dataset = build_dataset(cfg.data.model_init)  # FewShotVOCDataset
        print(f"type(model_init_dataset) = {type(model_init_dataset)}")
        # disable dist to make all rank get same data
        print("Building model_init_dataloader...")
        # torch.utils.data.dataloader.DataLoader
        model_init_dataloader = build_dataloader(
            ...
            dist=False,
            shuffle=False)
        print(f"type(model_init_dataloader) = {type(model_init_dataloader)}")
    ...
    if not distributed:
        ...
        print("Initializing model (forward_model_init in detector)...")
        # mmfewshot/detection/apis/test.py
        if cfg.data.get('model_init', None) is not None:
            print("cfg.data.get('model_init', None) is not None")
            from mmfewshot.detection.apis import (single_gpu_model_init,
                                                  single_gpu_test,
                                                  single_gpu_model_init_custom)
            # single_gpu_model_init(model, model_init_dataloader)
            single_gpu_model_init_custom(model, model_init_dataloader)
        else:
            print("cfg.data.get('model_init', None) is None")
            from mmdet.apis.test import single_gpu_test
        print("model_init done...")
        # print("model.inference_support_dict:")
        # for class_id, v1 in model.inference_support_dict.items():
        #     print(f"  class_id = {class_id}:")
        #     print(f"    res4_roi_feats: v1['res4_roi_feats'].size()")
